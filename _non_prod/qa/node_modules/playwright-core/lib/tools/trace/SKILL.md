...
description: Inspect Playwright trace files from the command line â€” list actions, view requests, console, errors, snapshots and screenshots.
...
3. Use `trace action <action-id>` to drill into a specific action â€” see parameters, logs, source location, and available snapshots.
...
All commands after `open` operate on the currently opened trace â€” no need to pass the trace file again. Opening a new trace replaces the previous one.
