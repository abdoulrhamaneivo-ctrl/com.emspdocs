...
export interface Browser {
   ...
   * the `context` is closed gracefully and any artifactsâ€”like HARs and videosâ€”are fully flushed and saved.
   *
   * **Usage**
   *
   * ```js
...
export interface BrowserType<Unused = {}> {
   ...
   * [browserType.connect(endpoint[, options])](https://playwright.dev/docs/api/class-browsertype#browser-type-connect),
   * which requires the major/minor client/server version to match (1.2.3 â†’ is compatible with 1.2.x).
   *
   * **Usage**
   *
...
export interface Debugger {
   ...
   * statement â€” it pauses execution at the call site immediately. On the contrary,
   * [debugger.requestPause()](https://playwright.dev/docs/api/class-debugger#debugger-request-pause) is equivalent to
   * "pause on next statement" â€” it configures the debugger to pause before the next action is executed.
   */
