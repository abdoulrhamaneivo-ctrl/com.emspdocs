...
if (empty($_FILES['image']) || $_FILES['image']['error'] !== UPLOAD_ERR_OK) {
    ...
    echo json_encode(['error' => 'Aucun fichier reÃ§u ou erreur upload']);
    exit;
}
...
$destDir = __DIR__ . '/../uploads/media/institution';
if (!is_dir($destDir)) {
    if (!mkdir($destDir, 0755, true)) {
        http_response_code(500);
        echo json_encode(['error' => 'Impossible de crÃ©er le dossier d upload']);
        exit;
    }
}
...
if ($realDir === false) {
    http_response_code(500);
    echo json_encode(['error' => 'Dossier de destination introuvable']);
    exit;
}
// chmod dÃ©sactivÃ© sur hÃ©bergement mutualisÃ© (ProFreeHost gÃ¨re les permissions).
if (!is_writable($realDir)) {
    http_response_code(500);
    echo json_encode(['error' => 'Dossier upload non accessible en Ã©criture']);
    exit;
}
...
if (!move_uploaded_file($_FILES['image']['tmp_name'], $destPath)) {
    http_response_code(500);
    echo json_encode(['error' => 'Ã‰chec sauvegarde fichier']);
    exit;
}

echo json_encode(['url' => 'uploads/media/institution/' . $filename]);
exit;
